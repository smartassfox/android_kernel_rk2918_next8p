...
#if defined(CONFIG_MACH_RK29_ODYS_NEOX8) || defined (CONFIG_MACH_RK29_NEXT8P)
...
static void ft5x0x_power_en(struct ft5x0x_data *ft5x0x, int on)
{
	...
	if (on) {
		gpio_direction_output(ft5x0x->touch_en_gpio, TOUCH_EN_LEVEL);
		gpio_set_value(ft5x0x->touch_en_gpio, TOUCH_EN_LEVEL);
		mdelay(10);
	} else {
		gpio_direction_output(ft5x0x->touch_en_gpio, !TOUCH_EN_LEVEL);
		gpio_set_value(ft5x0x->touch_en_gpio, !TOUCH_EN_LEVEL);
...
static int scan_screen_key(int x, int y, rect rt[])
{
	const int span = 10;
	int idx;
...
static int get_screen_key(int x, int y)
{
	...
	if (g_vid == VID_YM) {
		rect rt[] = {
			 ...
			 {834,	54,	KEY_HOMEPAGE},        	/* home */ 
			 {834,	90,	KEY_MENU},      	  	/* menu */ 
		...
		rect rt[] = {
			 {850,	128, 	KEY_BACK},  /* back */ 
			 {850,	95,	KEY_MENU},        /* home */ 
			 {850,	55,	KEY_HOMEPAGE},		/* menu */ 
			 {850,	5,	KEY_SEARCH},    /* search */ 
			 {0,0,0},
		};
		...
		rect rt[] = {
			{850,	128, 	KEY_SEARCH},  /* search */ 
			{850,	95,	KEY_HOMEPAGE},        /* home */ 
			{850,	55,	KEY_MENU},      	  /* menu */ 
			{850,	5,	KEY_BACK	},    /* back */ 
			{0,0,0}, 
		}; 
...
static int ft5x0x_process_points(struct ft5x0x_data *data)
{
	...
	if(g_vid == VID_DSW) {
		if(buf[1]) {
			if(buf[1] == 0x01)
				g_screen_key = KEY_SEARCH;	/* search */
			else if(buf[1] == 0x02)
				g_screen_key = KEY_HOME;	/* home */
			else if(buf[1] == 0x04)
				g_screen_key = KEY_F1;		/* menu */
			else if(buf[1] == 0x08)
				g_screen_key = KEY_BACK;	/* back */
			//else
			//	g_screen_key = 0;
...
#ifdef TOUCHKEY_ON_SCREEN
			if ( x >= (SCREEN_MAX_X + 10) && (status != 1)) {
				//printk("%d-%d(%d,%d)%d-%d\n", id, status, x, y, p, w);
				DBG("TOUCH_NO=%d: ID=%d,(X=%d,Y=%d), status=%d, pressure=%d, w=%d\n", i, id, x, y, status, 0, 0);
				if (g_screen_key = get_screen_key(x, y)) {
