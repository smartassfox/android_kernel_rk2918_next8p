...
static int mma8452_get_data(struct i2c_client *client)
{
	...
	x = mma8452_convert_to_int(buffer[1]);  
	y = mma8452_convert_to_int(buffer[0]);
...
/* Le seguenti righe funzionano con le lib del Prestigio 
	axis.x = mma8452_convert_to_int(buffer[1]);
	axis.y = mma8452_convert_to_int(buffer[0]);
	axis.z = mma8452_convert_to_int(buffer[2]);
Termine righe Prestigio 

 Le seguenti righe funzionano con le lib dell'easypad 
        axis.x = mma8452_convert_to_int(buffer[0]);
	axis.y = mma8452_convert_to_int(buffer[2]);
	axis.z = -mma8452_convert_to_int(buffer[1]);

 Termine righe easypad 
